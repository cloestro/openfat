...
//TODO: return the coordinates Sh_m, ... and
        ...
        double steptime, double R, double *Cs_i, double *Cs_ni)
{
    ...
    double sm11, sm22, sm33, sm12, sm23, sm31, T, Sh_m,
    ...
    sm12 = 0.0;
    Sh_m = 0.0;
    T = Sigma->tab[rows-1][0] - Sigma->tab[0][0];
    ...
    for (i=0; i < rows - 1; i++)
    {
        ...
        Sh_m += ( ( Sigma->tab[i][1] + Sigma->tab[i][2] + Sigma->tab[i][3] +
            Sigma->tab[i+1][1] + Sigma->tab[i+1][2] + Sigma->tab[i+1][3] ) / 3.0 ) *
            ( Sigma->tab[i+1][0] - Sigma->tab[i][0] ) / ( 2.0 * T ) ;
    ...
    A = ( sqrt(6.0) * tau_d / sigma_d - sqrt(2.0) ) *
        (1 - R ) / ( 1 + R );
   ...
   if (of_coordN(Sh_m, tau_oct_a_intr, A, B, &Sh_N_intr , 
               &tau_N_intr, &ON_intr) )
   {
       return -3 ;
   }
   if (of_coordN(Sh_m, tau_oct_a_max, A, B, &Sh_N_non_intr , 
...
#ifdef DEBUG
   printf("A=%f, B=%f\n", A, B);
   printf("Shm=%f, Tintr=%f, Tnintr=%f\n", Sh_m, tau_oct_a_intr,
           tau_oct_a_max);
   printf("ON_i=%f, ON_ni=%f\n", ON_intr, ON_non_intr);
#endif
    *Cs_i = of_coefsecur(Sh_m, tau_oct_a_intr, ON_intr);
    *Cs_ni = of_coefsecur(Sh_m, tau_oct_a_max, ON_non_intr);
